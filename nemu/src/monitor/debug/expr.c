...
enum {
	...
	/* TODO: Add more token types */
        , NUM, NEQ, OR, AND, REG, REF, NEG, SYM
};
...
static int find_dominated_op(int s, int e, bool *success) {
	...
	for(i = s; i <= e; i ++) {
		switch(tokens[i].type) {
			case REG: case NUM: case SYM: break;
	...
	if(s > e) {
		...
		*success = false;
		...
		switch(tokens[s].type) {
			...
			case SYM: val = look_up_symtab(tokens[s].str, success);
					if (!*success) { return 0; }
					break;
			default: assert(0);
		}
...
uint32_t expr(char *e, bool *success) {
	...
	for(i = 0; i < nr_token; i ++) {
		if(tokens[i].type == '-') {
			...
			if( !(prev_type == ')' || prev_type == NUM || prev_type == REG || prev_type == SYM) ) {
				tokens[i].type = NEG;
			}
		}
